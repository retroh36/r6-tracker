// ================================================================
// PLAYER PROFILE + AI COACHING REPORT
// ================================================================
function ProfileScreen({ onRoute, user, justUploaded }) {
  const [tab, setTab] = useState('coach');
  const [animate, setAnimate] = useState(justUploaded);
  const report = window.DATA.COACH_REPORT;
  const p = user;

  useEffect(() => {
    if (justUploaded) setTimeout(() => setAnimate(false), 1200);
  }, [justUploaded]);

  return (
    <>
      <TopNav route="profile" onRoute={onRoute} user={user} />
        ...
        <div className="wrap">
          ...
          {justUploaded && (
            <div style={{display:'flex',alignItems:'center',gap:12,padding:'12px 16px',border:'1px solid var(--good)',background:'oklch(0.780 0.155 155 / 0.06)',marginBottom:24}}>
              ...
              <div style={{fontFamily:"'JetBrains Mono'",fontSize:11,color:'var(--good)',textTransform:'uppercase',letterSpacing:'0.18em'}}>EXTRACT COMPLETE · COACH REPORT GENERATED IN 14.2S</div>
            </div>
          )}
          ...
          <div className="corners" style={{background:'var(--panel)',border:'1px solid var(--line)',padding:28,marginBottom:32,position:'relative',display:'grid',gridTemplateColumns:'auto 1fr auto',gap:28,alignItems:'center'}}>
            <Corners />
            <div className={"av xl " + p.avatar.theme}>{p.avatar.initials}</div>
            ...
            <div>
              ...
              <div style={{fontFamily:"'Space Grotesk'",fontWeight:700,fontSize:'clamp(32px,4vw,48px)',letterSpacing:'-0.03em',lineHeight:1}}>{p.username}</div>
              <div style={{display:'flex',gap:20,marginTop:12,flexWrap:'wrap'}}>
                <span style={{fontFamily:"'JetBrains Mono'",fontSize:11,color:'var(--fg-dim)',letterSpacing:'0.14em'}}>HANDLE · <b style={{color:'var(--fg)'}}>{p.handle}</b></span>
                <span style={{fontFamily:"'JetBrains Mono'",fontSize:11,color:'var(--fg-dim)',letterSpacing:'0.14em'}}>PLATFORM · <b style={{color:'var(--fg)'}}>{p.platform}</b></span>
                <span style={{fontFamily:"'JetBrains Mono'",fontSize:11,color:'var(--fg-dim)',letterSpacing:'0.14em'}}>RANK · <b style={{color:'var(--accent)'}}>{p.rank}</b></span>
                <span style={{fontFamily:"'JetBrains Mono'",fontSize:11,color:'var(--fg-dim)',letterSpacing:'0.14em'}}>ARCHETYPE · <b style={{color:'var(--accent)'}}>{p.archetype}</b></span>
              </div>
            </div>

            <div style={{display:'flex',flexDirection:'column',gap:10}}>
              <button className="btn primary" onClick={()=>onRoute('dashboard')}><span>BACK TO SQUAD</span><span className="arrow">→</span></button>
              <button className="btn ghost" onClick={()=>onRoute('upload')}>RE-UPLOAD STATS</button>
            </div>
          </div>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(6,1fr)',gap:0,border:'1px solid var(--line)',marginBottom:40}} className="stat-strip">
            <style>{`
              ...
              @media (max-width:520px){ .stat-strip{grid-template-columns:repeat(2,1fr) !important} .stat-strip > div{border-right:1px solid var(--line-dim) !important} .stat-strip > div:nth-child(2n){border-right:0 !important} }
            `}</style>
            <StatCell label="K/D RATIO" value={p.stats.kd} accent />
            <StatCell label="WIN RATE"  value={p.stats.winRate} unit="%" />
            <StatCell label="HEADSHOT"  value={p.stats.headshot} unit="%" />
            <StatCell label="MATCHES"   value={p.stats.matches} />
            <StatCell label="HOURS"     value={p.stats.hoursTotal.toLocaleString()} sub={`+${p.stats.hoursSeason} this season`} />
            <StatCell label="AVG DMG"   value={p.stats.avgDamage} />
          </div>
          ...
          <div className="tabs" style={{marginBottom:28}}>
            ...
            <div className={"tab " + (tab==='compare'?'active':'')} onClick={()=>setTab('compare')}>VS SQUAD</div>
            ...
            <div className="anim-fade">
                ...
                <div className="card">
                  ...
                  <div style={{display:'grid',gap:14}}>
                    {report.strengths.map((s,i) => (
                      <div key={i} style={{display:'grid',gridTemplateColumns:'22px 1fr',gap:12,paddingBottom:14,borderBottom:i<2?'1px dashed var(--line-dim)':0}}>
              ...
              <div className="card" style={{marginBottom:28}}>
                ...
                <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:14}} className="rec-grid">
                  ...
                  {report.recommendedOps.map((op,i) => (
          ...
          {tab === 'ops' && (
            <div className="card anim-fade">
              <div className="card-h"><div className="t">TOP OPERATORS · <b>BY PLAYTIME</b></div><div className="r">SEASON Y11</div></div>
              <div style={{display:'grid',gap:10}}>
                ...
                {p.topOps.map((op,i) => (
                  <div key={op.name} style={{display:'grid',gridTemplateColumns:'48px 1fr 100px 80px 80px 160px',gap:14,alignItems:'center',padding:'8px 0',borderBottom:'1px dashed var(--line-dim)'}}>
                    <div className={"av md " + ['h-r','h-b','h-g','h-a','h-p'][i]}>{op.name.slice(0,2)}</div>
                    <div style={{fontFamily:"'Space Grotesk'",fontWeight:600,fontSize:15}}>{op.name}</div>
          ...
          {tab === 'compare' && (
            <div className="card anim-fade">
              ...
              <div style={{display:'grid',gap:18,paddingTop:8}}>
                {[
                  ['K/D RATIO', p.stats.kd, p.squadAvg.kd, 2.5],
                  ['WIN RATE', p.stats.winRate, p.squadAvg.winRate, 100],
                  ['HEADSHOT %', p.stats.headshot, p.squadAvg.headshot, 100],
                  ['AVG DAMAGE', p.stats.avgDamage, p.squadAvg.avgDamage, 200],
                ].map(([l,y,t,m]) => (
                  <div key={l} style={{display:'grid',gridTemplateColumns:'140px 1fr 120px 80px',gap:18,alignItems:'center'}}>
                    <div style={{fontFamily:"'JetBrains Mono'",fontSize:11,color:'var(--fg-mute)',letterSpacing:'0.14em'}}>{l}</div>
                    <CompareBar you={(y/m)*100} them={(t/m)*100} />
                    <div className="mono" style={{fontSize:13,color:'var(--accent)',fontWeight:600}}>YOU {y}{l.includes('%')?'%':''}</div>
                    <div className="mono" style={{fontSize:11,color:'var(--fg-dim)'}}>AVG {t}{l.includes('%')?'%':''}</div>
                  </div>
                ))}
              </div>
            </div>
