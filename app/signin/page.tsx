...
export default function SignInPage() {
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'grid', gridTemplateColumns: '1fr 1fr', position: 'relative', overflow: 'hidden' }} className="signin-root">
      ...
      {/* RIGHT — form */}
      <div style={{ padding: 56, display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', zIndex: 1 }}>
        <form onSubmit={submit} className="corners" style={{ width: '100%', maxWidth: 440, background: 'var(--panel)', border: '1px solid var(--line)', padding: 36, position: 'relative' }}>
          ...
          <div style={{ display: 'grid', gap: 18 }}>
            <div className="field">
              ...
              <input className="input mono" type="email" placeholder="callsign@squad.gg" value={email} onChange={e => setEmail(e.target.value)} autoComplete="email" required />
            </div>
            <div className="field">
              ...
              <input className="input mono" type="password" placeholder="••••••••••" value={password} onChange={e => setPassword(e.target.value)} autoComplete={mode === 'signin' ? 'current-password' : 'new-password'} required />
              {mode === 'signup' && (
